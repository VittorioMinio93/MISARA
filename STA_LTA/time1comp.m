...
function time1comp(Timesel,Name,path,mainpath,fs,wlong,pathout,outputformat,text1,ii,bb)
...
for kk=1:length(station)
    ...
    if length(p)~=0
        ...
        if length(Z)==round(fs)*2*wlong+1
            ...
            output=strcat(pathout,juliand,'/',sta_ref,'.',Name(end-18:end-10),timeref,outputformat);if ~exist(strcat(pathout,juliand,'/'), 'dir');mkdir(strcat(pathout,juliand,'/'));end 
