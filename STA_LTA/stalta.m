...
% TIME_sel: selelction of corrected trigger times (s)
% Name,NAME: file name vector/matrix
% Name_sel: file names selected 
% family, Family: Family label 
% Z,Y,YY: extracted waveform vectors
% Z,N,E: the three components
% STA,LTA: Short and Long-terms windows
...
YY=[];
Timesel=[];
Namesel=[];wlong=[];wshort=[];f1=[];f2=[];
%% STA-LTA figure
fig_color= [.94 .94 .94];
panel_color= [.97 .97 .97];

avfig= figure('Name', 'STALTA analysis', 'position',get(0,'screensize'), ...
...
%% Text boxes
%-------------------------------- STA/LTA setting     
uicontrol(pan1,'Style','text', 'String','STA/LTA setting',...
...
function openfnc(~,~)
    ...
    RATIO=[];NAME=[];TIME_DET=[];NEWTIME_SEL=[];YY=[];Namesel=[];Timesel=[];%Initialize the main vectors
...
function calc(~,~)
    ...
    try
        ...
        if wshort<=0;set(text1, 'String','Error. Invalid analysis window.');drawnow;return;end
        if wlong<=0 | wshort>=wlong;set(text1, 'String','Error. Invalid analysis window.');drawnow;return;end
        ...
        RATIO=[];%Initialize the STA/LTA ratio vector
        ...
        for i=1:len 
            ...
            [Ratio,Time_trigger,Name]=stalta_alg(z,wshort,wlong,dt,fs,timemain,name);
            %Set the temporal format
            Time_trigger.Format='dd-MMM-yyyy HH:mm:ss.SSS';
            %Concatenate the results
            RATIO=[RATIO;Ratio];
            TIME_DET=[TIME_DET;Time_trigger];
            NAME=[NAME;Name];
        end
        %Plotting of the trigger times 
        ploting1(TIME_DET,RATIO,treshold,axes1)
        ...
        if length(z)<=round(wlong*fs);set(text1, 'String','Error. Invalid analysis window.');drawnow;return;end
...
function calc2(~,~)
    try
        ...
        idx=RATIO>=treshold;
        Ratio=RATIO(idx);
        nratio=length(Ratio);% number of selected triggers
        Timesel=TIME_DET(idx);Timesel.Format='dd-MMM-yyyy HH:mm:ss.SSS';
        Namesel=NAME(idx,:);
        nratio=length(Ratio); 
        YY=cell(1,nratio); %Initialize the extracted waveforms vector
        NEWTIME_SEL=cell(nratio,1);%Initialize the corrected trigger time vector
        ...
        for kk=1:nratio
            ...
            elseif TP_sampl-w1<0 & TP_sampl+w1<=length(z)
                ...
                newfile=strcat(path,'/',juliandnew,'/',params.sta_ref,'.BH',component,'.',num2str(year(newtime)),'.',juliandnew,'.',newtime2,'0000.sac');
        ...
        %Error assessment
        if isempty(cell2mat(ff));set(text1, 'String','Error. No files are loaded.');drawnow;return;end
        if isempty(Timesel);set(text1, 'String','Error.Triggers matrix is empty.');drawnow;return;end
...
%==========================================================================
%%%%%%%%%%%%%%%%%%%%STA/LTA algorithm%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Ratio,Time,Name]=stalta_alg(Z,wshort,wlong,dt,fs,timemain,name)
    Ratio=[];%Initialize the STA/LTA ratio vector
    Time=[];%Initialize the trigger time vector
    Name=[];%Initialize the filename vector
    %Set the analysis parameter
    N=length(Z);wl=round(wlong/dt); wl2=round(wshort/dt);
    %Initialize the winndow limits and the loop increment
    i1=0; i2=0;
    ii1=0; ii2=0;
    k=0;
    ...
    while i2 < N-wl || i2== N-wl
        k=k+1;
        i1=(1+(k-1)*wl); i2=i1+wl-1;%Limits of the LTA window
        y1=Z(i1:i2).*tukeywin(wl);%tapered cosine window
        ii1=i2-wl2+1;ii2=i2;%Limits of the STA window
        yy1=Z(ii1:ii2).*tukeywin(wl2);%tapered cosine window
        %Calculation of the STA and LTA terms
        LTA=sqrt(sum((y1.^2)/wl));
        STA=sqrt(sum((yy1.^2)/wl2));
        %Calculation of the STA/LTA ratio
        ratio=STA/LTA;
        %Calculation of the trigger time
        timetrig=seconds(ii1/round(fs));
        time=timemain+timetrig;
        %Concatenate the results
        Ratio=[Ratio;ratio];
        Time=[Time;time];
        Name=[Name;name];
...
%==========================================================================
%%%%%%%%%%%%%%%%%Plotting of the trigger times%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
function ploting1(time,ratio,treshold,ax)
    try
        cla(ax)%Clear the axes
        %Plot the sta/lta ratio values
        hold(ax,'on')
        plot(ax,time,ratio)
        %Plot the threshold line
        plot(ax,time,ones(length(ratio),1).*treshold)
        xlim(ax,[min(time) max(time)])
        ylabel(ax,'STA/LTA Ratio')
        hold(ax,'off')
    catch
        %Error assessment 
...
function ploting2(ii,ax)
    try
        wlong=str2num(get(h.wlong,'string'));%Get the long term window
        cla(ax)%Clear the axes
        %%Select the ii-th trigger time and set temporal limits of the extracted waveform 
        t1=Timesel(ii)-seconds(wlong);
        t2=Timesel(ii)+seconds(wlong);
...
function stlt1(~,~)
    %Error control
    if nratio==0;set(text1, 'String', 'Error. Signals matrix is empty.');return;end
...
function stlt2(~,~)
    ...
    slidval=slidval+1;if slidval>nratio;slidval=nratio;end
    %Plot the selected waveform 
    ploting2(slidval,axes3);
...
function save1(~,~)
    ...
    try
        ...
        plot(TIME_DET,RATIO)
        %Plot the threshold line
        plot(TIME_DET,ones(length(RATIO),1).*treshold)
        ...
        %Error assessment
        if size(TIME_DET,1)~=size(RATIO,1);set(text1, 'String','Error. Incoherent dimensions of the Triggers array.');drawnow;return;end
    end    
end
...
function save4(~,~) 
    ...
    if f2<=0 | f2<=f1;set(text1, 'String','Error. Invalid analysis frequencies.');drawnow;return;end
    if f1>=fs/2 | f2>fs/2;set(text1, 'String','Error. Invalid analysis frequencies.');drawnow;return;end
    wls=str2num(get(h.wpol,'string'));if wls<=0 | wls>=(wlong+wlong+dt);set(text1, 'String','Error. Invalid analysis window.');drawnow;return;end
    try
        ...
        switch params.comp_chan
            case'Comp'
                ...
                t1=time-seconds(wlong);
                t2=time+seconds(wlong);
                %Select the three components of  the waveforms detected
                [Z,N,E]=select3comp(time,Namesel(ii,:),mainpath,fs,wlong,params.sta_ref,text1);
...
%==========================================================================
%%%%%%%%%%%%%%%Save the results of the STA/LTA analysis %%%%%%%%%%%%%%%%%%%
function savetrig(~,~)
    text1.String=[];set(text1);drawnow;%Clear the Window message
    %Validation of the results
    if isempty(params.save);set(text1, 'String', sprintf('Error. No output directory.'));drawnow;return;end  
    if isempty(Timesel);set(text1, 'String', sprintf('Error. Triggers matrix is empty.'));drawnow;return;end
    %Create the Output directory and filename
    if ~exist([params.save '/STA_LTA/'], 'dir');mkdir([params.save '/STA_LTA/']);end
    output=[params.save '/STA_LTA/' 'Time_triggers' params.outputformat];
    %Create a table of the results
    data=table(datenum(Timesel'));data.Properties.VariableNames={'Time'};data.Properties.VariableUnits = {'datenum'};
    %Save the results
...
function savesel(~,~)
    ...
    pathout=[params.save '/STA_LTA/' Family '/'];
        ...
        case 'Man'
            ...
            switch params.comp_chan
                case 'Chan'
                     ...
                     time1comp(time,Namesel(ii,:),path,mainpath,fs,wlong,pathout,outputformat,text1,ii,length(NEWTIME_SEL))            
                case 'Comp'
                    %Save the ii-th waveforms detected (component system)
                    time3comp(time,Namesel(ii,:),path,mainpath,fs,wlong,pathout,outputformat,text1,ii,length(NEWTIME_SEL))
            end
        case 'Aut'
            %Automatic saving
            %Execute one of two groups of routines (Channel/Component system)
             switch params.comp_chan
                case 'Chan'
                    ...
                    for kk=1:length(NEWTIME_SEL)
                        ...
                        time1comp(time,Namesel(kk,:),path,mainpath,fs,wlong,pathout,outputformat,text1,kk,length(NEWTIME_SEL))                        
                    end                    
                case 'Comp'
                    %Loop through the number of triggers 
                    for kk=1:length(NEWTIME_SEL)
                        ...
                        time3comp(time,Namesel(kk,:),path,mainpath,fs,wlong,pathout,outputformat,text1,kk,length(NEWTIME_SEL))                        
                    end
             end
    end
end
