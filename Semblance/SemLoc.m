%%%%%%%%%%%%% Calculation of the Semblance function %%%%%%%%%%%%%%%%%%%%%%%
% February 2021 
% Vittorio Minio
%
%%%%%%%%%%%%%%%VARIABLES%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% params: user parameters
% avfig: Semblance figure
...
% w_sem: the analysis window for the Semblance analysis (s)
...
avfig= figure('Name', 'Semblance Analysis', 'position',get(0,'screensize'), ...
...
% YZ plane axes
axes2= axes('position',[0.41 0.68 0.4 0.28]);
hold(axes2,'on')
grid(axes2,'on')
grid(axes2,'minor')
colorbar(axes2)
...
% XZ plane axes
axes3= axes('position',[0.41 0.14 0.4 0.28]);
hold(axes3,'on')
grid(axes3,'on')
grid(axes3,'minor')
colorbar(axes3)
...
%% Text boxes
%-------------------------------- Semblance settings
uicontrol(pan1,'Style','text', 'String','Semblance setting',...
...
uicontrol(pan1,'Style','text', 'String','Win (s)',...
    ...
    'BackgroundColor',panel_color);
uicontrol(pan1,'Style','text', 'String','Cf (Hz)',...
    'HorizontalAlignment','left',...
    'Units','normalized','Position',[.03 .70 .4 .02],...
    'BackgroundColor',panel_color);
uicontrol(pan1,'Style','text', 'String','Q',...
    'HorizontalAlignment','left',...
...
uicontrol(pan1,'Style','text', 'String','Vel (km/s)',...
    ...
    'BackgroundColor',panel_color);
uicontrol(pan1,'Style','text', 'String','n',...
    'HorizontalAlignment','left',...
...
h.f0= uicontrol(pan1,'Style','edit', 'String',params.f0,...
    'HorizontalAlignment','left',...
    'Units','normalized','Position',[.28 .69 .2 .04],...
    'BackgroundColor','w','callback',@deflimits);
h.Q= uicontrol(pan1,'Style','edit', 'String',params.Q,...
    'HorizontalAlignment','left',...
...
h.vel_sem= uicontrol(pan1,'Style','edit', 'String',params.vel_sem,...
    ...
    'BackgroundColor','w','callback',@deflimits);
h.n_sem= uicontrol(pan1,'Style','edit', 'String',params.n_sem,...
    'HorizontalAlignment','left',...
...
h.rad= uibuttongroup(pan1,'units','normalized','BackgroundColor',panel_color,...
    'bordertype','none','Position',[.04 .82 .5 .05]);
...
% Editable texts
uicontrol(pan1,'Style','text', 'String','Semblance',...
    'HorizontalAlignment','left','Fontweight','bold',...
...
%==========================================================================
%%%%Calculation and plot of the Semblance function%%%%%%%%%%%%%%%%%%%%%%%%%
function calc(~,~) 
    text1.String=[];set(text1);drawnow;%Clear the Window message
    cla(axes1);cla(axes2);cla(axes3)%Clear the axes
    title(axes1,ttit);%Clear the title
    ...
    try
        ...
        f0=str2num(get(h.f0,'string'));if f0<=0;set(text1, 'String','Error. Invalid analysis frequencies.');drawnow;return;end
        Q=str2num(get(h.Q,'string'));if Q<=0;set(text1, 'String','Error. Invalid parameter value.');drawnow;return;end
        v=str2num(get(h.vel_sem,'string'));if v<=0;set(text1, 'String','Error. Invalid velocity value.');drawnow;return;end
        n_sem=str2num(get(h.n_sem,'string'));if n_sem<=0 | n_sem>1;set(text1, 'String','Error. Invalid parameter value.');drawnow;return;end
        freq_sem=deblank(get(h.freq_sem,'string'));
        freq=split(freq_sem,'-');
        f1=str2num(freq{1});f2=str2num(freq{2});
        ...
        switch params.comp_chan
            case 'Chan'
                staidx=find(strcmp(station,[params.sta_ref '.BH' comp ]));
            case 'Comp'
                staidx=find(strcmp(station,params.sta_ref ));
        end
        %Picking routine 
        [TRIGG,YY,XSS,YSS,ZSS,Name]=SelectSignal(len,NStation,f1,f2,staidx,delim);
        w1=round(w_sem*fs);
        %Loop through the number of files selected
        for i=1:len
            ZZ=YY{i};%Select the i-th waveforms
            XS=XSS{i};YS=YSS{i};ZS=ZSS{i};%Select the i-th station coordinates
            ...
            for k=1:ntrig
                ...
                [valor,ix1,ix2,ix3,loc]=Semblance(ZZ,Xq,Yq,Zq,NStation,v,Nx,Ny,Nz,XS,YS,ZS,fs,trig,staidx,w1,K,n_sem);
                J_error=[NaN NaN NaN];%Initialize the standard error of the JackKnife estimator
                %Active/Deactive the the JackKnife routine 
                if emod=='y'
                    [J_error]=JackKnife(ZZ,Xq,Yq,Zq,xq,yq,zq,NStation,v,Nx,Ny,Nz,XS,YS,ZS,fs,trig,staidx,w1,ix1,ix2,ix3,K,n_sem);
                end
...
%==========================================================================
%%%%%%%%%%%%%%%%Picking routine %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [TRIG,Yy,XSS,YSS,ZSS,Namemain]=SelectSignal(len,NStation,f1,f2,staidx,delim)
    try
        Yy=[];%Initialize the events vector
        TRIG=[];trig=0;%Initialize the picking times vectors
        ...
        for i=1:len
            ...
            name=file_name(end-18:end-4);%Create the term of research
            ...
            list=searchAllStaz(pathname,station,component,name,params.comp_chan);nlist=length(list);
            %Control the calculation through the number of stations
            if (nlist==NStation)
                XS=[];YS=[];ZS=[];ZZ=[];NAME=[];%Initialize the main vectors
                %Loop through the number of stations used
                for jj=1:nlist
                    %Load the jj-th seismic trace  
                    fsign=strcat(pathname,list(jj).name);
                    load(fsign)
                    z=mytrace.data;fs=mytrace.sampleRate;dt=1./mytrace.sampleRate;fn=fs/2;staname=split(mytrace.station,' ');staname=staname{1};
                    %Redefine the station name 
                    if params.comp_chan=='Chan'
                        staname=[staname '.' mytrace.channel];
                    end
                    ...
                    idx=strcmp(station,staname);
                    xss=xs(idx);yss=ys(idx); zss=zs(idx);XS=[XS;xss];YS=[YS;yss];ZS=[ZS;zss]; ZZ=[ZZ,Z];NAME=[NAME,{staname}];    
                end
                %Picking mode 
...
%==========================================================================
%%%%Plot of XY section of Semblance in UTM system %%%%%%%%%%%%%%%%%%%%%%%%%
function ploting1(ii,ax)
    ...
    %Select the ii-th maximum values of analysis grid (x,y,z,Semblance value)
    ix1=LOCAL(ii,1);
    ix2=LOCAL(ii,2);
    ix3=LOCAL(ii,3);
    valor=LOCAL(ii,4);
    ...
    %Plot of XY section of Semblance  
    C=squeeze(loc(:,:,ix3)); 
    hold(ax,'on') 
...
%==========================================================================
%%%%Plot of XY section of Semblance in wgs84 system %%%%%%%%%%%%%%%%%%%%%%%
...
%==========================================================================
%%%%Plot of YZ section of Semblance in UTM system %%%%%%%%%%%%%%%%%%%%%%%%%
function ploting2(ii,ax)
    ...
    zmin=str2num(get(h.zmin,'string'));
    zmax=str2num(get(h.zmax,'string'));
    %Select the ii-th 3D Semblance grid
    loc=cell2mat(GRID(ii));
    %Select the ii-th maximum values of analysis grid (x,y,z,Semblance value)
    ix2=LOCAL(ii,2);
    valor=LOCAL(ii,4);
    cla(ax)%Clear YZ plane axes
    %Plot of YZ section of Semblance 
    colormap(ax,'parula');
    C=squeeze(loc(:,ix2,:)); 
...
%==========================================================================
%%%%Plot of YZ section of Semblance in wgs84 system %%%%%%%%%%%%%%%%%%%%%%%
...
%==========================================================================
%%%%Plot of XZ section of Semblance in UTM system %%%%%%%%%%%%%%%%%%%%%%%%%
function ploting3(ii,ax)
    ...
    %Plot of XZ section of Semblance
    colormap(ax,'parula');
    C=squeeze(loc(ix1,:,:)); 
...
%==========================================================================
%%%%Plot of XZ section of Semblance in wgs84 system %%%%%%%%%%%%%%%%%%%%%%%
...
function deflimits(~,~)
        ...
        f0=str2num(get(h.f0,'string'));
        Q=str2num(get(h.Q,'string'));
        vel_sem=str2num(get(h.vel_sem,'string'));
        n_sem=str2num(get(h.n_sem,'string'));
        ...
        if (size(f0,1)==0);set(h.f0,'string',params.f0);end
        if (size(Q,1)==0);set(h.Q,'string',params.Q);end
        if (size(vel_sem,1)==0);set(h.vel_sem,'string',params.vel_sem);end
        if (size(n_sem,1)==0);set(h.n_sem,'string',params.n_sem);end
...
function save1(~,~)
    ...
    try
        ...
        switch params.coord_sys
            case 'm'
                new_figure=figure('units','normalized','outerposition',[0 0 1 1],'Name','Semblance Grid','NumberTitle','off');
                subplot(2,2,1)
                %Plot of XY section of Semblance
                C=squeeze(loc(:,:,ix3));
                hold('on') 
        ...
        switch params.coord_sys
            case 'm'
                ...
                patch(isocaps(XXXq,YYYq,ZZZq,S,isoval),...
                    ...
                    'EdgeColor','none')
                hold on 
                %Plotting the location of the maximum Semblance value
...
%==========================================================================
%%%%Save the results of the Semblance analysis%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function save_sem(~,~)
    text1.String=[];set(text1);drawnow;%Clear the Window message
    ...
    for kk=1:size(LOCAL,1)
        ...
        %Compute the temporal variables 
         ...
         pathout=[params.save '/Semblance/' Family '/' jday '/'];if ~exist(pathout, 'dir');mkdir(pathout);end 
    ...
    %Command message
    set(text1, 'String', sprintf(['Semblance' params.outputformat '-Observed data are saved.']));
    drawnow
end
end