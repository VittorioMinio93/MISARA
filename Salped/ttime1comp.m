%%%%%%%%%%%%% Save the waveforms detected (channel system) %%%%%%%%%%%%%%%%
% February 2021 
% Vittorio Minio
%
%%%%%%%%%%%%%%%VARIABLES%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function ttime1comp(Timesel,Name,path,mainpath,fs,wlong,wshort,pathout,outputformat,text1,ii,bb)
try
    ...
    for kk=1:length(station)
        ...
        filesac=strcat(mainpath,'/',juliand,'/',sta_ref,Name(end-19:end));
        ...
        if length(p)~=0
            ...
            elseif TP_sampl-w1<0 & TP_sampl+w2<=length(z)
                ...
                newfile=strcat(mainpath,'/',juliandnew,'/',sta_ref,'.',num2str(year(newtime)),'.',juliandnew,'.',newtime2,'0000.sac');
            ...
            if length(Z)==round(fs)*(wshort+wlong)+1
                ...
                output=strcat(pathout,juliand,'/',sta_ref,'.',Name(end-18:end-10),timeref,outputformat);
                mytrace.data=Z;mytrace.sampleCount=length(Z);mytrace.startTime=datenum(Timesel-seconds(wshort));mytrace.endTime=datenum(Timesel+seconds(wlong));
                ...
                elseif outputformat=='.txt' 
                    Time=datenum([Timesel-seconds(wshort):seconds(dt):Timesel+seconds(wlong)]);
